...
public class SuperLoadingProgress extends View {
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		mRectF.set(new RectF(radius + strokeWidth, radius + strokeWidth, 3 * radius + strokeWidth, 3 * radius
				+ strokeWidth));
		// 初始化下落路径
	...
	private void drawSmallRectFly(Canvas canvas) {
		...
		float x2 = (float) (bigRadius * Math.cos(curSweepAngle + 0.05 * endAngle + 0.1 * endAngle
				* (1 - curSweepAngle / 0.9 * endAngle)));//
		float y2 = -(float) (bigRadius * Math.sin(curSweepAngle + 0.05 * endAngle + 0.1 * endAngle
				* (1 - curSweepAngle / 0.9 * endAngle)));
	...
	private void drawRectDown(Canvas canvas) {
		...
		// 非交集
		Region region1 = new Region(Math.round(pos1[0]) - strokeWidth / 4, Math.round(pos1[1]), Math.round(pos2[0]
				+ strokeWidth / 4), Math.round(pos2[1]));
		region1.op(mRect, Region.Op.DIFFERENCE);
		drawRegion(canvas, region1, downRectPaint);

		// 交集
		Region region2 = new Region(Math.round(pos1[0]) - strokeWidth / 2, Math.round(pos1[1]), Math.round(pos2[0]
				+ strokeWidth / 2), Math.round(pos2[1]));
	...
	private void drawFork(Canvas canvas) {
		...
		canvas.drawLine(2 * radius + strokeWidth, radius + strokeWidth, 2 * radius + strokeWidth, 2 * radius
				+ strokeWidth, downRectPaint);
	...
	private void drawTick(Canvas canvas) {
		Path path = new Path();
		/*
		 * On KITKAT and earlier releases, the resulting path may not display on
		 * a hardware-accelerated Canvas. A simple workaround is to add a single
		 * operation to this path, such as dst.rLineTo(0, 0).
		 */
